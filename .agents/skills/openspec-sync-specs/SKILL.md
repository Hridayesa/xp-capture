...
description: Sync delta specs from an OpenSpec change to main specs. Use when the user wants to update main specs with changes from a delta spec, without archiving the change. Also use when the user says "bun run tools/openspec.ts sync" or "opsx sync".
...
1. **Select the change**
   ...
   - If ambiguous, run `bun run tools/openspec.ts list --json` to get available changes and ask the user to select one
...
4. **For each delta spec, apply changes to main specs**
   ...
   - If archive invoked this workflow inline and supplied a valid snapshot from
     `bun run tools/openspec.ts instructions specs --change "<name>" --json`, reuse it and do not
   ...
   b. **Read the main spec** at `<planningHome.root>/openspec/specs/<capability-path>/spec.md` (may not exist yet)
      ...
      **If it does not exist yet** (a new capability), match what `bun run tools/openspec.ts archive` does:
   ...
   c. **Apply changes intelligently**:
      ...
      **`## Purpose` in the delta:**
      - The main spec already has one and it is authoritative - leave it alone
        (this is what `bun run tools/openspec.ts archive` does; it warns and moves on)
   ...
   d. **Create new main spec** if capability doesn't exist yet:
      - Only when the delta has ADDED requirements to put in it and no MODIFIED or
        ...
        `bun run tools/openspec.ts archive` rejects it with `Spec must have at least one requirement`.
      ...
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `bun run tools/openspec.ts archive` does); only write a brief TBD placeholder when it does not
      - Add Requirements section with the ADDED requirements
      - Follow the **Main Spec Format Reference** below

5. **Validate updated main specs**

   Run `bun run tools/openspec.ts validate --specs` with the same selected-root flags used earlier.
...
- A MODIFIED block carries the whole requirement - body plus every scenario that survives the change. `bun run tools/openspec.ts validate` and `bun run tools/openspec.ts archive` both reject one that drops a scenario the main spec still has.
